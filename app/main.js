...
const refreshFunction = () => {
    if (localStorage.getItem("Prayer")) {
        ...
        const nowTime = toSeconds(to24Hour(time.toLocaleTimeString("en-US")));
...
const refreshInterval = setInterval(() => refreshFunction(), 20);
...
// Function to fetch suggestions - function created with help of chatgpt
document.getElementById('address').addEventListener("input", function() {
  ...
  const geocodeUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=jsonv2&limit=5`;
...
// Fetch coordinates for selected address - function created with help of chatgpt
document.getElementById('geocodeForm').addEventListener("submit", function(event) {
  ...
  const geocodeUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=jsonv2`;
...
document.querySelector("#setZoom").onclick = (e) => {
    e.preventDefault();
    zoom();
    fullscreen(); // Function Declared on HTML document
}
